...
fn main() {
	size_x := os.input('X размер: ').int()
	size_y := os.input('Y размер: ').int()
	assert size_x > 0
	assert size_y > 0
	// 0 - стены
	// 1 - проход
	// 2 - генерация
	// 4 - граница
	println('( ) простая генерация')
	println('(с) слайд-шоу')
	println('(н) генерация с начальной')
	println('(п) генерация с путём')
	mode := os.input('=')
	match mode {
		'с' {
			mut gen := gen_init(size_x, size_y)
			wait := os.input('миллисекунд на фото: ').int()
